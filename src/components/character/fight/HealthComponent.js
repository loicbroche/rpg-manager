  ...
  render() {
    ...
    return (
      <div className="healthComponent" title="Niveau d'énergie">
        {onChange && <span className={`currentModifier decrease-value ${value===0 &&"disabled"}`} onClick={() => {this.handleValueChange(-1)}} title="Diminuer le niveau dénergie"></span>}
        ...
        {onChange && <span className={`currentModifier increase-value ${value===MAX_VAL &&"disabled"}`} onClick={() => {this.handleValueChange(1)}} title="Augmenter le niveau d'énergie"></span>}
      </div>
    )
  }
